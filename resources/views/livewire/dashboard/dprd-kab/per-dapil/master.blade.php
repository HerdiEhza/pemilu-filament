<div class="py-12">
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="justify-center overflow-hidden bg-white shadow-sm sm:rounded-lg">
            ...
            <select wire:model="kabKotaActive" name="kabKotaActive">
            ...
            <div class="relative p-4 overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($dapils as $data)
                            <tr
                                class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{-- <a href="{{ route('dashboard.per-kab_kota', preg_replace('/[^A-Za-z0-9-]+/', '-', $data->name)) }}"> --}}
                                    <a href="{{ route('dashboard.per-dapil', ['dapil' => $data->id]) }}">
                                        {{ $data->nama_dapil }}
                                    </a>
                                </th>
                                @foreach ($partais as $partai)
                                    @foreach ($suaras as $suara)
                                    @if ($suara->data_partai_id == $partai->id and $suara->data_dapil_id == $data->id)
                                        <th class="px-6 py-3">
                                            <a href="">
                                                {{ $suara->total_suara ?? '0' }}
                                            </a>
