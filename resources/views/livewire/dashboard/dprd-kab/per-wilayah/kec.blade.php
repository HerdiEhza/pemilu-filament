                        ...
                        @foreach ($kelurahans as $data)
                            <tr
                                ...
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <a href="{{ route('dashboard.per-kel', ['prov' => $this->provinsiActive, 'kab_kota' => $this->kabKotaActive, 'kec' => $this->kecamatanActive, 'kel' => $data->id]) }}">
                                        {{ $data->name }}
                                    </a>
                                </th>
                                @foreach ($partais as $partai)
                                    @foreach ($suaras as $suara)
                                    @if ($suara->data_partai_id == $partai->id and $suara->tps_kel_id == $data->id)
                                        <th class="px-6 py-3">
                                            <a href="">
                                                {{ $suara->total_suara ?? '0' }}
                                            </a>
